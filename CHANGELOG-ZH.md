...
- 强类型 `Array` 与全部 `PackedArray` 的下标读取、写入及复合赋值统一经过支持负下标的边界检查，同时保持先求接收者、再求下标的执行顺序；异常消息数据现在产生受控脚本诊断，不再直接解引用原生存储并终止进程。
- 附着式脚本的 `self` 跨越强类型原生调用边界时保留类型正确的 `RefCounted` 强引用，覆盖嵌套消息对象和生成的协议模型，不再执行不安全的 owner 转换。
- 发行运行时 ABI 升级至 12，旧 SDK 或复制不完整的 SDK 会在导出预检阶段被拒绝，不再链接生命周期不兼容的生成代码。
- 导出运行回归新增二进制消息头、嵌套分发表、`Callable` 处理器、信号、`@onready` 场景数组、继承与内部类 ready 生命周期以及 128 条连续消息压力路径。
