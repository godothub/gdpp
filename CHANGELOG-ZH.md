...
- 新增 lowerer、verifier 和完整项目编译回归，直接覆盖嵌套终止分支裁剪以及恶意越界/乱序 ID；本地编译器单元测试为 553 / 553，core CTest 为 21 / 21。
- 使用 Windows 11 x86_64、MSVC 与官方 Godot 4.6.1，从清理缓存后的发行插件安装流程完成导入、Release AOT、原生编译、链接、导出和独立运行；导出包审计 391 个文件、加载 42 个资源、确认唯一项目库且违规为 0。
- Windows 回环协议认证覆盖 HTTP 登录、四份 JSON、WebSocket 二进制帧、protobuf、远程头像、两个礼物平台、点赞、英雄、积分、分组同步、97 包礼物突发、服务端正常关闭、短帧及截断/损坏消息；全部场景保持对象池总数 5000、进程退出 0，损坏消息的受控源码错误与原生 GDScript 位于同一边界。
...
- 不放宽任何阈值即把 fault-safe 后端恢复到商业 10% 性能合同：清缓存的官方 Godot 4.7.1 Universal 2 source/AOT 差分完整匹配 65 个 fault 调用者和 39 项值哈希，5 轮性能矩阵通过启动、固定帧及全部 13 个 family；Dictionary、String 与 Variant 运算分别比 GDScript 快 38.57%、6.87% 和 28.74%。
...
- await 默认参数与协程访问器 AOT oracle 改为由用户命令行参数选择的独立导出进程，并拒绝这些日志中的所有意外诊断；这既保留进程级运行覆盖，也符合官方 release 模板默认 `disable_path_overrides=true`、明确关闭 Godot `--script` 路径覆盖的安全合同。
