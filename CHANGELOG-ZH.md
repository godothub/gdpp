...
- 为每种桌面宿主内置固定版本的 Ninja 1.13.2，并让生成的客户构建只接受经过校验的 Ninja 图，移除已退役的串行任意命令计划。
- 每次导出均生成完整的当前项目 C++ 状态，相同输出保留时间戳，将项目 manifest 收敛为所有权清单，并让 Ninja 成为原生增量构建的唯一权威。
- 以 CPU 和内存预算并行构建彼此独立的生成 C++ 翻译单元，使用编译器 depfile、分阶段顺序链接与打包、确定性命令输入、工具链身份失效和零工作增量复用。
- 从隐藏进程持续上报单调的逐文件编译与链接进度，同时保留精确 MSVC 环境、UTF-8 编译响应文件、Unicode 与长路径以及 Windows Emscripten 命令包装。
- 每次构建前由主线程固定编辑器路径与扩展契约，在隔离工作线程运行前端和 Ninja 工具链，并通过主线程队列传递进度，避免跨线程调用 Godot 或 GDScript。
- 在发行汇总中强制校验 Ninja 版本、提交、宿主矩阵、元数据、可执行权限、非 ASCII 安装项目导出、失败恢复和无源码交付边界。
- 通过经过校验的同卷事务提交 iOS XCFramework，并支持按内容复用与中断替换恢复。
- 项目、生成物、工作副本、SDK、响应文件和制品路径在全部编译器边界统一使用 UTF-8，Windows 导出不再依赖活动 ANSI 代码页。
- Windows 工具链输出捕获以 Ninja 进程生命周期为边界，持续驻留的 MSVC 辅助进程不再通过继承管道句柄阻塞已完成的导出。

## 1.8.4

- 新增面向 Godot 4.6～4.7 的 `gdpp-lite.zip`，保留 macOS、Windows 编辑器与桌面导出及 Android、Web 导出，不包含 Linux 和 iOS 载荷。
- 导出工作副本不再包含易变的 Godot 编辑器、导出及着色器缓存；仅对必要的导入和脚本元数据执行有界一致性检查与原子提交，避免并发缓存更新破坏重复 AOT 导出。
...
- 在并发线程加载、路径、UID、`load`/`preload`、缓存、附加、继承、回滚和终止过程中保持已编译 Script 的唯一身份。
...
- 编译脚本资源在赋值和显式转换中完整保留 GDScript、Script、Resource、RefCounted 与 Object 身份。
- 编辑器专用资源图遵循客户导出排除规则；任何实际选入运行包的编辑器专用资源仍会被严格拒绝。
