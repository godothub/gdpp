...
- Defer non-trivial per-thread debugger state until the first Godot callback instead of placing Godot values in the Windows DLL static TLS initialization table. A Windows-only loader gate now opens the compiler with `LoadLibraryExW` before invoking the GDExtension entry, verifies `gdpp_library_init`, and unloads it; the same-machine differential reproduces error 1114 with the unfixed artifact and loads the corrected MSVC artifact cleanly.
