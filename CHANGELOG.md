...
- Support genuinely suspending static functions with an owner-free completion object, typed results, and Signal resumption, and support typed coroutine lambdas with per-call suspension state and independently ordered concurrent resumptions.
...
- Run suspending engine virtual continuations through the coroutine runtime, validate every immediate non-Variant native return before crossing the engine ABI, and gate the behavior in exported official-Godot runs; non-immediate typed virtual return waiting remains an explicit pre-release P0 audit item.
- Preserve `@static_unload` intent from AST through HIR, generated metadata, project manifests, cache identity, and editor Script descriptors; independently observable per-script unload/reset remains an explicit pre-release P0 audit item.
