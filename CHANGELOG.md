...
- Add a whole-function, symbol-identity-based proof for in-place compound updates of local untyped Dictionary literal slots. Reassignment, aliases, calls, subscripts, unknown keys, typed storage, method access, and closure capture all invalidate the proof and retain the checked runtime path.
- Restore the fault-safe backend to the commercial 10% performance contract without relaxing a threshold: a clean official Godot 4.7.1 Universal 2 source/AOT differential matches all 64 fault callers and the 39-value hash, while its five-round performance matrix passes startup, fixed-frame, and all 13 families; Dictionary, String, and Variant operations measure 5.60%, 10.16%, and 0.37% faster than GDScript.
