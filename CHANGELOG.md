...
- Route statically known `Dictionary.key` reads through the same checked keyed runtime as subscripts: a missing or invalid key now terminates the current function, a present key containing `null` remains a valid value, and a fallible receiver is checked before the lookup. The source/AOT fault oracle covers both cases and typed compound-key failures instead of allowing a silent default Variant.
- Restore the fault-safe backend to the commercial 10% performance contract without relaxing a threshold: the rebuilt official Godot 4.7.1 Universal 2 differential passes behavior, startup, fixed-frame, and all 13 families; a five-round local candidate measures Dictionary 7.40% and String 11.56% faster than GDScript, with Variant operations 1.61% slower.
