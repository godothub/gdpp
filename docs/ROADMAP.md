...
6. 编译翻译单元由内置 Ninja 按 CPU 与可用内存预算并行；链接和平台合包保持有序单任务。
...
“生产主路径可用”不等于 1.0 Stable；它表示当前 1.9.x 发布门禁覆盖的目标可以失败关闭交付。

## 已关闭：M1～M4 P0 功能闭环

1.8.0 以“实现、官方源/AOT 差分、生成 C++17、真实 Godot 运行和发行审计”同时成立作为关闭
条件；1.9.0 继续用真实跨脚本工程补强已关闭范围，而不是以单个样例成功作为完成证据。
...
- `M5-003` 生成输出使用临时文件和原子提交；补齐断电、低磁盘、只读、跨卷和崩溃测试。
- `M5-004` 持久化 AST/符号摘要；新进程在源码和能力表未变时不重复完整前端。
- `M5-005` 增加 toolchain doctor、问题面板跳转、Ninja 失效原因报告和脱敏诊断包。
- `M5-006` 完成空格、非 ASCII、Unicode 规范化、Windows 长路径、符号链接和大小写矩阵。

导出翻译单元已由内置 Ninja 在 CPU/内存预算内有限并发；链接和目标合包通过单深度池保持顺序。
剩余工作是用户取消、超时和中止恢复，不再维护第二套自定义并行调度器。
