...
| M1 | 词法与语法 | 接近完成 | `breakpoint`、可恢复解析、持续 fuzz、完整源范围 golden |
...
“生产主路径可用”不等于 1.0 Stable；它表示当前 1.7.x 发布门禁覆盖的目标可以失败关闭交付。
...
- `M1-001` 为 `breakpoint` 建立 token、AST、HIR/MIR 节点和明确的导出/调试行为；如果 Godot
  导出模板无调试器端点，必须定义可验证的 no-op/诊断策略。
...
`.gdpp` 不进入当前 1.7.x 功能闭环。启动该里程碑前必须先完成：
