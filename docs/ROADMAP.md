...
| M4 | C++17 后端/runtime | 主路径完成 | 异步虚函数完整等待 ABI、全部 API/容器失败矩阵、符号映射与二进制审计 |
...
- `M4-001` `void` 异步引擎回调 continuation 和受检立即返回已经进入真实运行门禁；继续定义
  非立即返回值的外部调用者等待 ABI，不能复用不兼容的普通脚本方法协议。
