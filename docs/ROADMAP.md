...
| M2 | 类型与语义 | 主路径完成 | 动态错误全量差分、真实多 peer RPC、动态脚本路径、custom/double SDK |
| M3 | HIR/MIR 与优化 | 部分完成 | MIR 稳定 Value ID/所有权下沉、数据流优化、IR 序列化与等价性门禁 |
| M4 | C++17 后端/runtime | 主路径完成 | 全部 API/容器失败矩阵、符号映射、边界故障注入与二进制审计 |
...
- `M2-001` 真正挂起的静态函数协程使用无实例的独立完成 owner，保留类型化返回和 Signal 恢复。
- `M2-002` lambda 使用 GDScript 创建时捕获快照和独立调用帧，覆盖类型化协程、并发恢复、嵌套
  返回闭包，以及通过共享 `Array[Callable]` 实现的递归。
- `M2-003` Signal/Callable 运行矩阵覆盖 bind/unbind、一次性/延迟/引用计数连接、发射期间连接
  变更、宿主销毁、Callable 身份及多线程调用。
- `M4-001` 挂起调用使用 GDScript 等价 FunctionState；普通调用、静态/lambda、内部访问器和
  异步引擎虚函数共享完成/恢复生命周期，并完成官方 Godot 4.7.1 差分。
- `@static_unload` 在 4.4～4.7 保留完整注解身份并匹配当前 Godot“脚本保持到语言关闭”的可观察
  行为；未来引擎修复引用归零卸载时按目标版本重新开启差分任务。
...
- `M3-001` HIR 局部声明、引用、捕获、await 提升和调试变量已经共享稳定 `FlowSymbolId`；继续把
  稳定 Value ID、所有权类别、可优化表达式和调用契约完整下沉到 MIR。
...
`M4-001` 已关闭：引擎不会等待脚本虚函数的协程结果；官方 GDScript 立即返回 FunctionState，
随后由虚函数 ABI 执行 Variant→native 转换。GDPP 已复现该对象、字符串、恢复、完成和生命周期
规则，不再维护一个不存在的“外部等待协议”。
