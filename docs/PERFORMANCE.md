...
```text
Dictionary mean: GDS 196.53 ns / AOT 120.72 ns（AOT -38.57%）
String mean:     GDS 137.28 ns / AOT 127.85 ns（AOT -6.87%）
Variant mean:    GDS 23.27 ns / AOT 16.58 ns（AOT -28.74%）
13/13 benchmark families、启动、固定帧：全部 <= 10%
行为 oracle：PASS
```

该结果来自 fault frame 最终效应分析、Dictionary 读写合同、精确整数槽位原地运算、Variant
边界借用及单次 RHS 快照移动提交版本，用于证明完整错误传播没有依靠热点轮询换取正确性，也
没有通过放宽阈值掩盖回归。
...
```text
N 个生成/运行时翻译单元
  -> 内置 Ninja 在 CPU/内存预算内并行编译失效项
  -> 顺序执行 1 次链接或目标合包
```

并发数取逻辑 CPU、12 任务上限和可用内存预算的最小值；内存不足时自动退化为单任务。链接和
平台合包使用深度为 1 的池，并显式等待编译 barrier，因此不会出现两个链接器争用同一输出。
Windows MSVC 环境只初始化一次，所有子进程隐藏控制台。

### 增量构建

只有 Ninja 负责原生增量判断。前端每次建立完整项目语义图并生成全部目标内容；写入层对内容
寻址，相同 C++、头文件、符号元数据、命令和图文件不改变时间戳。项目 manifest 只负责生成输出
的所有权与陈旧文件回收，不缓存编译结果。

Ninja 使用 depfile 或 MSVC include 数据库跟踪真实头依赖，并把 SDK/runtime/bridge、工具链、
profile、命令文件和链接输入纳入图。方法体变化只重编译对应源文件；公开头变化由真实 include
边传播；静态库变化只重链；无变化导出复用全部对象。
...
1.9.0 插件发布资产大小门禁使用最近的 1.8.0 正式资产作为历史比较基线：
...
当前流水线不再发布这三个宿主专用包，而是生成包含 4.6～4.7 的 `gdpp.zip`、包含 4.4～4.7
的 `gdpp-all.zip`，以及包含 4.6～4.7 且移除 Linux/iOS 载荷的 `gdpp-lite.zip`。完整包合并三种
桌面编辑器和全部导出 SDK，lite 包保留 macOS、Windows、Android 与 Web；公共头文件/runtime
每个 Godot 版本只保留一份，每个平台/模式也只保留一份 `template_release`，不包含
template_debug/editor 静态绑定。每个桌面宿主额外保留一个固定版本的轻量 `gdpp-ninja`
执行器；它不按 SDK 版本复制。
