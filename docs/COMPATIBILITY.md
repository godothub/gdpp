...
本页描述 GDPP 1.8.3 能接受、生成、运行和交付的语言边界。总体状态和优先级见
...
`Dictionary.key` 与 `Dictionary[key]` 都执行受检键读取，并通过 named/keyed Variant ABI 在
一次查找中取得有效位。缺失键和强类型不兼容键设置当前函数 fault；键存在且值为 `null` 时返回
合法 `Nil`，不会误判为缺失。可失败的字典接收者在查找前完成一次求值和故障检查。

强类型 `Dictionary[K,V].key` 只在 `StringName` 可赋给 `K` 时成立，读取结果为 `V`。
直接写入的类型或只读错误终止当前函数；复合写入的非法强类型值按官方行为保持原值并继续，只读
写回仍终止。只有经符号身份和非逃逸证明的局部非强类型字面量已存在键会借用读取或原地更新，
其余全部保留受检读取/写回。精确整数槽位在公共 64 位合同内直接更新；除零/模零不提交结果并
报告原始 `.gd` 行列，非整数或不支持的运算继续使用 Godot Variant ABI。
