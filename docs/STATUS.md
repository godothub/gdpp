...
- 审计版本：GDPP 1.9.0。
- 功能审计范围：1.9.0 当前发布候选。
- 最近正式发布门禁：1.8.4 / GitHub Actions `30608539870`，发布成功。
- 本地编译器单元测试：由当前提交的完整 core 门禁执行，不在文档中固化易过时的数量。
- Godot 目标：4.4、4.5、4.6、4.7；CI 使用 4.4.1、4.5.2、4.6.3、4.7.1。
- SDK schema：12；生成项目 runtime ABI：23。
...
| 语义与生成 | 类型化 AST/HIR、显式 CFG/MIR、C++17、确定求值顺序、强类型容器、完整项目符号图和确定性再生成 |
...
| 发行物 | `gdpp.zip` 包含 Godot 4.6～4.7，`gdpp-all.zip` 包含 4.4～4.7，两者提供三桌面编辑器及全部导出 SDK；`gdpp-lite.zip` 包含 4.6～4.7，仅提供 macOS/Windows 编辑器与桌面导出及 Android/Web |

## P0 语言与运行语义结论

1.9.0 保持当前声明兼容面的 M1～M4 P0 阻断项关闭，并补齐真实跨脚本工程暴露出的 parser、
...
1. 同一项目被多个 Godot 编辑器或 CLI 同时构建时，没有跨进程排他锁；单进程内输出事务是
...
5. 描述符与场景转换具备事务恢复，仍缺断电、低磁盘、只读目录、跨卷 rename、超长路径和
   连续崩溃恢复矩阵。
6. 前端摘要尚未跨进程持久化；生成文件按内容保留、对象由 Ninja 精确复用，但新进程仍需重新词法、解析和语义分析。
...
- 稳定性：重复导出已覆盖编辑器缓存并发更新；仍缺多小时网络/场景切换/资源流送、高频重复导出、
  磁盘增长、内存峰值和恢复压力门禁。
...
- 不在客户导出时调用 CMake、Python、SCons 或系统 Ninja；原生构建只调用插件内置并校验版本的 Ninja。
