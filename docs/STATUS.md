...
- 审计版本：GDPP 1.8.1。
- 功能审计范围：1.8.1 发布分支当前提交；正式发布门禁由同一提交的 release workflow 执行。
...
1.8.1 保持当前声明兼容面的 M1～M4 P0 阻断项关闭，并把 MIR operation identity 延后到最终
CFG 裁剪和前驱重建之后统一规范化；嵌套终止分支不会再因临时汇合块被删除而留下 ID 空洞：
