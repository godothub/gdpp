...
1. `@static_unload` 已登记语法，但没有独立的可观察生命周期语义。扩展卸载会释放 GDPP 静态
   资源，不能把这一行为冒充完整注解兼容。
2. 真正挂起的 `await` 不能用于静态函数；`_init` 也不允许挂起。实例协程已经支持类型化返回，
   但静态协程仍会失败关闭。
3. lambda 已支持捕获、参数、默认值、返回类型和 fire-and-forget `await` 主路径；递归
   self-capture、完整带返回值 lambda 协程、复杂嵌套闭包生命周期尚未形成完整等价实现。
4. 异步覆盖引擎虚函数时，外部调用者如何等待返回值尚未形成完整 ABI；不能把实例方法协程的
   动态 Signal 协议直接套到所有引擎虚函数。
5. 动态强类型容器转换、除零、越界、失效 Object/Ref 和错误字典键虽然有安全 runtime，
   但部分失败位置、错误文本和“当前函数何时终止”的时序尚未与 GDScript VM 全量差分。
6. Signal/Callable 主路径已实现；递归闭包、完整 bind/unbind、连接/断开与对象销毁竞争仍缺
   生命周期压力矩阵。
7. RPC 注解与单进程 `call_local` 已实现；真实多 peer 的 authority 拒绝、可靠性/排序/丢包、
   断线重连和对象序列化压力尚未认证。
8. 字面量脚本路径的 `preload/load`、脚本工厂和 UID 可解析；运行时拼接出的 `.gd` 路径无法在
   已剥离源码的包中穷举。当前缺少显式动态脚本保留清单和可证明的路径集合分析。
9. 自定义 Godot 构建、double precision API、用户合并的 extension API 还没有正式 SDK
    生产与冲突校验流程。
