...
静态 `Dictionary.key` 和 `Dictionary[key]` 共享同一受检 keyed lookup。查找结果非 `Nil` 时
只执行一次哈希查询；结果为 `Nil` 时再用 `has()` 区分合法的“键存在且值为 `null`”与缺失或
强类型不兼容键。后者设置当前 fault frame，前者正常返回 `null`。若字典接收者本身可失败，
生成代码先固定并检查接收者，再执行键查找，因此不会用默认空字典覆盖原始故障。
