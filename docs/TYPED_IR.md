...
fault polling 是最后一条规则的显式实现，而不是例外。后端从类型化表达式的 kind、Type、
ResolutionKind、调用参数转换和赋值目标计算保守故障效应；效应只决定某个已经排好求值顺序的
结果后是否需要读取活动 fault state，不改变节点、控制边或副作用。任何未知、动态、严格转换、
容器访问、整数除法/取模、对象有效性或脚本重入边界都保留轮询；只有完整静态证明为无故障的
值流才能省略。编译器测试同时锁定“纯函数无伪轮询”和“动态参数转换/越界/除零仍有轮询”。

赋值 lowering 把接收者和 RHS 分别固定，RHS 始终只求值一次。RHS 快照在提交后不可观察，因此
后端可把它移动到转换结果和最终存储；若转换可能失败，则仍先物化结果、检查 fault，再修改目标。
`to_variant` 对已经是 Variant 的 lvalue 保留引用，对 rvalue 转移所有权，其他类型才创建边界值。
这些是 typed IR 求值/所有权信息的机械实现，不是基于源码文本或函数名的热点特判。

类型化 `Dictionary.key` 仍标记为可能失败，即使接收者类型已经确定为 Dictionary。语义层把
点号键建模为 `StringName`：若它不能赋给 `Dictionary[K,V]` 的 `K`，在进入 IR 前拒绝；否则
表达式结果保留 `V`。生成器先固定并轮询可失败接收者，再进入返回独立有效位的 named Variant
ABI；缺失键、非法强类型键和键存在但值为 `Nil` 不会被混淆。

局部 Dictionary 槽位优化不改变 typed IR。C++ 后端以 `FlowSymbolId` 对完整函数执行保守使用
审计；只有非强类型字面量的已知初始键，且容器从未重赋值、别名、逃逸、下标访问、调用方法或被
闭包捕获时，复合写入才取得原地 `Variant&`。证明失败只是选择受检 getter/setter，不会删除 IR
节点、求值或 fault 边界。
