...
同一完成协议，同时在创建/调用两个边界维持捕获快照。属性访问器等不向调用者暴露返回状态的
内部挂起点使用自动回收的私有状态。异步引擎虚函数与官方 GDScript 一样立即把 FunctionState
交给 Godot 的 Variant→native 转换，而不是让引擎阻塞等待。最终目标仍是把当前程序计数器与
结构化共享帧两条实现路径统一成显式协程帧。
