...
类型化 `Dictionary.key` 仍标记为可能失败，即使接收者类型已经确定为 Dictionary。生成器把
可能失败的接收者先固定到临时值并轮询，再进入与下标共享的 keyed runtime；运行时区分缺失键、
非法强类型键和键存在但值为 `Nil`，因此效应分析不能把点号语法误当成普通无故障字段读取。
